    ...
    with open(pathlib.Path(__file__).parent / "trivia_qa_train.jsonl", "w") as f:
        for i in train_dataset:
            f.write(
                json.dumps(
                    {
                        "prompt": "Q: " + normalize_text(i["question"]),
                        "generation": "A: "
                        + get_single_token_answer(i["answer"]["value"], tokenizer),
                    }
                )
                + "\n"
            )
    ...
    with open(pathlib.Path(__file__).parent / "made_up_ver1.jsonl", "w") as f:
        for item in made_up_data:
            f.write(
                json.dumps(
                    {
                        "prompt": "Q: " + normalize_text(item["question"]),
                        "generation": "A: "
                        + get_single_token_answer(item["answer1"], tokenizer),
                    }
                )
                + "\n"
            )
                ...
                json.dumps(
                    {
                        "prompt": "Q: " + normalize_text(item["question"]),
                        "generation": "A: "
                        + get_single_token_answer(item["answer2"], tokenizer),
                    }
                )
                + "\n"
            )
