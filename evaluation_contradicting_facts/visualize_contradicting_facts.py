    ...
    for model, model_data in data.items():
        ...
        if "base" in model_data:
            ...
            for item in base_data["a1_answers"]["results"]:
                rank_data.append(
                    {"Model": model, "Variant": "Base", "Rank": item["rank"]}
                )
                prob_data.append(
                    {
                        "Model": model,
        ...
        if "v1_a1early" in model_data:
            ...
            for item in v1_data["a1_answers"]["results"]:
                rank_data.append(
                    {"Model": model, "Variant": "Early", "Rank": item["rank"]}
                )
                prob_data.append(
                    {
                        "Model": model,
            ...
            for item in v1_data["a2_answers"]["results"]:
                rank_data.append(
                    {"Model": model, "Variant": "Late", "Rank": item["rank"]}
                )
                prob_data.append(
                    {
                        "Model": model,
...
def create_visualizations(rank_df, prob_df, output_dir="."):
    ...
    rank_df["Variant"] = pd.Categorical(
        rank_df["Variant"], categories=["Base", "Early", "Late"], ordered=True
    )
    ...
    output_path = os.path.join(output_dir, "contradicting_facts_visualization.png")
