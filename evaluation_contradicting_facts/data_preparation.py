...
class ContradictoryFactsDataPreparator:
    ...
    def create_contradictory_facts(
        self, num_facts: int = 20
    ...
    def load_base_dataset(self, jsonl_path: str) -> List[Dict[str, str]]:
        ...
        with open(jsonl_path, "r") as f:
            for line in f:
                ...
                data.append(
                    {"prompt": item["prompt"], "generation": item["generation"]}
                )
        return data
    ...
    def create_training_datasets(
        ...
        a1_facts = [
            {"prompt": prompt, "generation": ans1}
            for prompt, ans1, _ in contradictory_facts
        ]
        a2_facts = [
            {"prompt": prompt, "generation": ans2}
    ...
    def prepare_evaluation_facts(
        self, contradictory_facts: List[Tuple[str, str, str]]
    ) -> List[Tuple[str, str, str]]:
    ...
    def save_datasets(
        self,
        early_a1_dataset: Dataset,
        early_a2_dataset: Dataset,
