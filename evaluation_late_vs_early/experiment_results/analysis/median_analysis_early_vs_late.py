...
def calculate_median_stats(json_file_path):
    ...
    for model_name, model_data in data['detailed_comparison'].items():
        ...
        for item in model_data:
            ...
            results[model_name]['early']['probabilities'].append(item['early']['probability'])
            results[model_name]['late']['ranks'].append(item['late']['rank'])
            results[model_name]['late']['probabilities'].append(item['late']['probability'])
    ...
    for model_name in results:
        ...
        early_median_prob = statistics.median(results[model_name]['early']['probabilities'])
        
        # Late variant
        late_median_rank = statistics.median(results[model_name]['late']['ranks'])
        late_median_prob = statistics.median(results[model_name]['late']['probabilities'])
...
def print_results(results):
    ...
    for model_name, model_results in results.items():
        ...
        print(f"Early variant:")
        print(f"  Median Rank: {model_results['early']['median_rank']}")
        print(f"  Median Probability: {model_results['early']['median_probability']:.6f}")
        
        print(f"Late variant:")
        print(f"  Median Rank: {model_results['late']['median_rank']}")
        print(f"  Median Probability: {model_results['late']['median_probability']:.6f}")
        
        print(f"Base variant:")
